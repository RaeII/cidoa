...
- Câmera principal: `camera.far = HorizonSettings.renderDistance` (padrão `CITY_SCENE_CONFIG.far = 600`). Slider "Distância do horizonte" reescreve `far` + `updateProjectionMatrix()` e chama `environmentUpdater.setRadius` — céu fica a `renderDistance * 0.9`, sempre dentro do far plane. Também chama `fitGroundToHorizon()`: horizonte curto encolhe o chão cinza junto (meia-diagonal `size/2*sqrt(2)` fica dentro do far E da esfera do céu), senão sobraria plano cortado e vazio entre borda e céu.
- Chão local: `GroundSettings.size` (padrão `groundSize = 600`), acompanha câmera, dois triângulos e altura `−0.05`. Slider "Tamanho do chão" (aba horizonte) escala o plano em `updateGroundSettings`, mas passa por `fitGroundToHorizon()`: `size = min(GroundSettings.size, renderDistance * 1.25)`. Só limita — o valor do painel volta sozinho quando o horizonte é ampliado. Não toca em `camera.far`. Meia-diagonal `300*sqrt(2) ≈ 424` fica dentro do `far` padrão (600); a névoa apaga a borda bem antes dela. Coordenadas locais preservam folga de `0.01u` até terreno. Plano de um milhão de unidades foi removido: arredondamento Float32 ultrapassava essa folga e causava piscadas.
