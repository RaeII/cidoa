...
- Câmera principal: `camera.far = HorizonSettings.renderDistance` (padrão `CITY_SCENE_CONFIG.far = 600`). Slider "Distância do horizonte" reescreve `far` + `updateProjectionMatrix()` e chama `environmentUpdater.setRadius` — céu fica a `renderDistance * 0.9`, sempre dentro do far plane. Também chama `fitGroundToHorizon()`: o chão acompanha o horizonte (`setSpan(renderDistance * 2.2)`), então a borda do mesh continua além do far em qualquer alcance e a linha segue reta.
- Chão local: `PlaneGeometry` de lado `renderDistance * GROUND_SPAN` (`2.2`), acompanha câmera, dois triângulos e altura `−0.05`. Lado DERIVADO do horizonte, sem slider próprio. `updateGroundSettings` cuida só de cor/material; o lado vem de `fitGroundToHorizon()` → `groundPlane.setSpan(renderDistance * 2.2)`. Meio lado `1.1*far > far` garante que a borda do mesh caia sempre além do far plane: a linha do horizonte é o corte do far plane (distância constante → reta), nunca a silhueta do plano. Borda do mesh a `1.1*far` (660 no padrão), sempre fora do alcance — nunca desenhada. A névoa, que só depende da distância, chega saturada no corte do far plane e suaviza a linha. Coordenadas locais preservam folga de `0.01u` até terreno. Plano de um milhão de unidades foi removido: arredondamento Float32 ultrapassava essa folga e causava piscadas.
