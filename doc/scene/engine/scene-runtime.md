...
- Montanhas: mesma malha, seed, cores, posição e resolução — alterar distância não regenera relevo. O cull entra por `terrainRig.setRenderDistance` + `updateCulling` (discard no fragmento, ver [[scene-builders#`createTerrain.ts`]]); `setCullEnabled(false)` durante a captura do cube devolve o relevo inteiro ao probe.
