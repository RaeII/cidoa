...
- Câmera principal: `camera.far = HorizonSettings.renderDistance` (padrão 600). Atualiza projeção e raio do céu (`renderDistance * 0.9`). Alcance dos edifícios independente.
...
Verificação: `node scripts/check-horizon.mjs`. Runtime real com I/O substituído; valida três modos, 14.265 raios em órbita/aspectos/zoom/distâncias, borda horizontal, ausência da camada cinza adicional, culling de 500 prédios (slider dos prédios não pode encolher o chão da cidade), montanhas intactas, alcance do probe, precisão Float32 em órbita e descarte de recursos. Sem servidor/navegador; não mede FPS da aplicação.
