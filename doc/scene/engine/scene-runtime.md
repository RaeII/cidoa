...
5. **Resolução dinâmica** — ajusta `renderScale` para atingir `targetFps`. **Desligada no padrão** (piso `renderScaleFloor = 1`, controlado pelo granulado do painel)
...
O `renderScale` é multiplicado pelo `devicePixelRatio` (limitado pelo `dprCap`, `2` no padrão). O piso da queda é `renderScaleFloor`, não `minRenderScale`: começa em `CITY_SCENE_CONFIG.minRenderScale` e é reescrito por `setGrain`. Com piso `1` os dois ramos ficam presos em `1` e nenhum `setPixelRatio` roda — o render nunca sai da resolução nativa.

#### Granulado (`setGrain`)

Granulado = queda de resolução sob carga, visível como imagem granulada. Controle na aba **tela** do [[html-components|CityControlPanel]], estado `grain` do `CitySceneEditor`, **padrão `0`**.

```
renderScaleFloor = 1 - clamp(grain, 0, 1) * 0.5
grain 0   → piso 1.00 → downscale desligado, nunca granula
grain 1   → piso 0.50 → troca nitidez por FPS de forma agressiva
```

Baixar o granulado sobe `renderScale` até o novo piso **na hora** (`setPixelRatio` + `setSize` imediatos) — sem isso a imagem ficaria granulada até o FPS se recuperar sozinho. Mudanças têm cooldown de 1,5s; o delta imediatamente posterior à captura do cubemap não entra na média, evitando que um frame esporádico de CubeCamera/PMREM deixe a resolução permanentemente baixa em monitores de 60 Hz.
...
type CitySceneRuntime = {
  ...
  setFacadeTexturePool(keys: readonly string[]): void  // texturas sorteáveis por edifício (catálogo)
  ...
  setGrain(grain: number): void // 0 = resolução nativa travada; >0 libera o downscale por FPS
...
- Câmera principal: `camera.far = HorizonSettings.renderDistance` (padrão `CITY_SCENE_CONFIG.far = 600`). Slider "Distância do horizonte" reescreve `far` + `updateProjectionMatrix()` e chama `environmentUpdater.setRadius` — céu fica a `renderDistance * 0.9`, sempre dentro do far plane. Não mexe no chão: o alcance do chão tem slider próprio (`groundDistance`), aplicado no mesmo `updateHorizonSettings` via `groundPlane.setSpan(groundDistance * 2)`.
- Chão local: `PlaneGeometry` de lado `groundSpanOf(horizon) = groundDistance * 2`, acompanha câmera, dois triângulos e altura `−0.05`. Lado vem do slider "Distância do chão" (`HorizonSettings.groundDistance`, padrão `far * 1.1` = 660); `updateGroundSettings` cuida só de cor/material, `updateHorizonSettings` chama `groundPlane.setSpan(...)`. Plano centrado na câmera → meio lado = `groundDistance` = raio em que o chão acaba. Enquanto `groundDistance > far` a borda cai além do far plane em qualquer direção horizontal e nunca é desenhada: a linha do horizonte é o corte do far plane (distância constante → reta), nunca a silhueta do plano. Baixar abaixo do `far` traz a borda do quadrado pra imagem — chão acabando antes do céu, escolha do usuário. A névoa, que só depende da distância, chega saturada no corte do far plane e suaviza a linha. Coordenadas locais preservam folga de `0.01u` até terreno. Plano de um milhão de unidades foi removido: arredondamento Float32 ultrapassava essa folga e causava piscadas.
...
- Edifícios: `distance`/`backDistance` continuam no `setRenderDistance` (só o manager — o relevo saiu desse limite); cull a cada 0.25s compacta instâncias e controla formatos customizados/acessórios. `renderDistance` não toca nesse cull.
- Chão da cidade: lotes, calçadas, postes e asfalto usam o **mesmo** `distance`/`backDistance`, no mesmo passe de 0.25s (compactação de instâncias — ver [[scene-managers#Cull do chão da cidade]]). Antes o loteamento seguia desenhado onde os prédios já tinham sumido.
- Montanhas: mesma malha, seed, cores, posição e resolução — alterar distância não regenera relevo. O cull entra por `terrainRig.setRenderDistance` (alimentado por `renderDistance * TERRAIN_CULL_SPAN`, **não** por `distance`/`backDistance` — o teste é radial e o arco tem que ficar fora do frustum) + `updateCulling` (discard no fragmento, ver [[scene-builders#`createTerrain.ts`]]); `setCullEnabled(false)` durante a captura do cube devolve o relevo inteiro ao probe.
